...
//                h_data[i] = 1.0f;
    ...
    // printf("sum: %f\n", reference[num_elements - 1] + h_data[num_elements - 1]);
    ...
    pre_sums = allocate(num_elements, level);
    ...
    prescanArray(d_odata, d_idata, num_elements, pre_sums);
    // **===-----------------------------------------------------------===**
    ...
    //     printf("%f%c", reference[i], i == num_elements - 1 ? '\n' : ' ');
    // printf("computed\n");
    // for (int i = 0; i < num_elements; i++)
    //     printf("%f%c", h_data[i], i == num_elements - 1 ? '\n' : ' ');
