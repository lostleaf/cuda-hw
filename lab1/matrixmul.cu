...
void output_matrix(const Matrix M)
{
    for (unsigned int i = 0; i < M.height; ++i)
        for (unsigned int j = 0; j < M.width; ++j) 
            printf("%lf%c", M.elements[i * M.width + j], j == M.width - 1 ? '\n' : ' ');
}
////////////////////////////////////////////////////////////////////////////////
// Program main
////////////////////////////////////////////////////////////////////////////////
...
Matrix init_device_matrix(const Matrix M, bool is_memcpy)
{
	int sizeM = M.width * M.height * sizeof(float);
	Matrix d_M = M; 
	d_M.elements = NULL;
	cudaMalloc((void **) &d_M.elements, sizeM);
	if (is_memcpy)
    	cudaMemcpy(d_M.elements, M.elements, sizeM, cudaMemcpyHostToDevice);
    return d_M;
}
...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
	...
	dim3 blockSize(M.height, N.width);
    Matrix d_M = init_device_matrix(M, 1);
    Matrix d_N = init_device_matrix(N, 1);
    Matrix d_P = init_device_matrix(P, 0);

	MatrixMulKernel<<< 1, blockSize >>> (d_M, d_N, d_P);
	
	cudaMemcpy(P.elements, d_P.elements, P.width * P.height * sizeof(float), cudaMemcpyDeviceToHost);
